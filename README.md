...
The published page reproduces `tracker.md` without editorial changes. The build step only converts markdown to HTML, adds heading ids for the table of contents, opens external links in a new tab, wraps tables so they scroll horizontally, and attaches CSS classes to the author's own bold confidence labels (`HIGH`, `MED-HIGH`, `MED`, `SEARCH-QUALIFIED`, `LOW`) and status classes (`[PENDING]`, `[STALLED]`, `[FAILED]`). The page shows one section at a time with a per-section index, search across all entries, and collapsible record cards; with JavaScript off, every section is shown in full. The build fails if any table cell, any link (text and URL), or any word of the markdown is missing from the HTML.
