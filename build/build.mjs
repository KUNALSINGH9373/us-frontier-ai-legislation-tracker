...
const pairIds = new Set(pairsData.map((x) => x.id));
const yearsOf = (id) => (id.startsWith("hb-") || pairIds.has(id) ? "*" : entryYears[id] ? [...entryYears[id]].sort().join(" ") : "");
...
sections.forEach((sec) => { if (!sec.synthetic) sec.body = sec.body.replace(/<(article class="record"|div class="pair") id="([^"]+)"/g, (m, tag, id) => `<${tag} id="${id}" data-years="${tag.startsWith("div") ? "*" : yearsOf(id)}"`); });
...
const tableBody = `<header class="sec-head"><p class="sec-meta"><span class="sec-group">Start here</span><span class="sec-count">${tableRows.length} rows</span></p><h2 id="table">Comparison table of all entries</h2></header>
...
<li>A build script renders that markdown to this page. Each table row becomes one entry; each column becomes a labelled field under the tracker's own heading, in the tracker's order. The two comparison matrices in section K stay as tables. The exclusion list in section J is a definition list.</li>
<li>The document is split into sections at its own headings, grouped for navigation, and given a directory, jump lists, search and previous/next links. The confidence key and verification note live on this page.</li>
</ol>
<h4>Checks that run on every build</h4><ul>
...
<div class="about-block"><h3>The derived views</h3>
<h4>Map</h4><p>A relationship map drawn from the tracker's own statements: drafting families, predecessors and amendments, federal preemption and reciprocity, orders that drove later actions, litigation, the auditor layer, and named sponsors, supporters, opponents and parties. Each relationship is stored with the tracker clause it comes from, and the build refuses any relationship whose clause is not found verbatim in the tracker. Dashed lines are relationships the tracker itself qualifies. Lines carry no quantity and are drawn at one width.</p>
<h4>Changes over time</h4><p>Every date in the tracker's Signed, Effective, Status, Introduced and Date cells becomes an event carrying the entry it belongs to, the column it came from, the clause of text around it, and an event type inferred from the words in that clause. Where the tracker gives only month and day, the year is taken from the same cell and the event is marked as inferred. Hovering a mark shows the original cell text, and a table view lists every event.</p>
<h4>Comparison table</h4><p>Every entry as one row with the tracker's Sponsor, mechanism, threshold, status, source and confidence cells placed under their own headings, sortable and filterable, with a CSV download. Below it, a cross-check against the American Action Forum's list of federal AI bills records which bills appear in both, which AAF bills fall within or near the tracker's frontier scope but are not yet entries, and which tracker bills AAF lacks. Those candidates are for the tracker's verification process; they are not entries.</p>
<h4>Handbook</h4><p>The <em>Plain-English Handbook for the Frontier AI Law Audit (2025–26)</em>, a companion guide to the US legal system, legislative procedure, statutory reading, AI-governance vocabulary, enforcement, preemption, litigation terms and research method. Its text is shown verbatim, one chapter at a time; glossary entries become term cards, numbered procedures become step cards, and the ten closing rules become rule cards. Trailing colons on glossary terms are dropped in the card headings, and the build checks that every word of the handbook appears on the page.</p></div>
<div class="about-block" id="confidence-key"><h3>Confidence key, sources and updates</h3>
<p class="about-note">The following text is the tracker's own introduction, reproduced verbatim.</p>${overviewRest}
${linkCheck ? `<h4>Link check</h4><p>Every unique link in the tracker was requested on ${linkCheck.date}. ${linkCheck.ok} returned 200. ${linkCheck.forbidden} returned 403 from sites that block automated access, which matches the tracker's own note about such sites. ${linkCheck.notfound} returned 404. No link was changed as a result; the full report is in the repository.</p>` : ""}
<h4>Source and updates</h4><p>The markdown of record, the raw export, the build and the checks are in the <a data-site href="${REPO_URL}">repository</a>. To update, edit the markdown, run the build and the verifier, and push; the site redeploys. <a href="tracker.md">Download the markdown</a>.</p></div>`;
...
const glance = `<section class="panel snapshot" aria-labelledby="glance-h"><div class="panel-head"><h2 id="glance-h" class="panel-title">Snapshot</h2><p class="panel-note">Counts come from the tracker's sections and the status class written at the start of each status cell. The year chart counts instruments with at least one dated event in each year, one line per group; click a year to filter the whole tracker.</p></div>
...
<div class="yearblock"><h3 class="sub-title">Activity by year</h3>
${(() => { const groups = ["state", "federal", "exec", "context"]; const W = 640, H = 260, L = 36, R = 24, T = 18, B = 40; const xs = YEARS.map((_, i) => L + i * ((W - L - R) / (YEARS.length - 1))); const max = Math.max(1, ...groups.flatMap((g) => yearPanelData.map((d) => d.counts[g] || 0))); const top = Math.ceil(max / 5) * 5; const y = (v) => T + (H - T - B) * (1 - v / top); const ticks = [0, top / 2, top].map((v) => Math.round(v)); const grid = ticks.map((v) => `<line x1="${L}" x2="${W - R}" y1="${y(v)}" y2="${y(v)}" class="ygrid"/><text x="${L - 8}" y="${y(v) + 4}" class="yax" text-anchor="end">${v}</text>`).join(""); const lines = groups.map((g) => { const pts = yearPanelData.map((d, i) => [xs[i], y(d.counts[g] || 0), d.counts[g] || 0, d.y]); const path = pts.map((p, i) => (i ? "L" : "M") + p[0].toFixed(1) + " " + p[1].toFixed(1)).join(" "); const dots = pts.map((p) => `<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="4.5" fill="${GROUP_COLORS[g]}" class="ydot"><title>${p[3] === "2027+" ? "2027 and later" : p[3]} · ${GROUPS.find((x) => x.key === g).name}: ${p[2]}</title></circle>`).join(""); const last = pts[pts.length - 1]; const peak = pts.reduce((m, p) => (p[2] > m[2] ? p : m), pts[0]); const lab = `<text x="${peak[0].toFixed(1)}" y="${(peak[1] - 9).toFixed(1)}" class="ylab" text-anchor="middle" fill="${GROUP_COLORS[g]}">${peak[2]}</text>`; return `<path d="${path}" fill="none" stroke="${GROUP_COLORS[g]}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>${dots}${lab}`; }).join(""); const xlab = YEARS.map((yy, i) => { const total = Object.values(yearPanelData[i].counts).reduce((a2, b2) => a2 + b2, 0); return `<a href="#" class="ybar" data-year="${yy}" aria-label="Filter to ${yy === "2027+" ? "2027 and later" : yy}: ${total} instruments"><text x="${xs[i].toFixed(1)}" y="${H - 14}" class="xax" text-anchor="middle">${yy}</text><text x="${xs[i].toFixed(1)}" y="${H - 1}" class="xtot" text-anchor="middle">${total} total</text></a>`; }).join(""); return `<svg class="ylines" viewBox="0 0 ${W} ${H}" role="img" aria-label="Instruments with at least one dated event per year, one line per group">${grid}${lines}${xlab}</svg>`; })()}
<ul class="ylegend">${["state", "federal", "exec", "context"].map((g) => `<li><i style="background:${GROUP_COLORS[g]}"></i>${GROUPS.find((x) => x.key === g).name}</li>`).join("")}</ul>
<p class="panel-foot">Click a year to filter the whole tracker to it.</p></div></div>
</div></section>
<section class="panel whenwhere" aria-labelledby="states-h"><div class="panel-head"><h2 id="states-h" class="panel-title">Where and when</h2><p class="panel-note">Left: entries with a State column, across enacted laws, pending bills, IVO measures, precursors and adjacent laws. Right: the most recent dated events on or before the verification date, then the effective dates, deadlines and scheduled steps after it. Colour is the kind of event; * marks a year inferred from the same cell.</p></div>
<div class="whenwhere-grid">
<div><h3 class="sub-title">State measures</h3><ul class="statebars">${states.map(([st, n]) => `<li><span class="st">${st}</span><span class="bar"><span style="width:${(n / stateMax) * 100}%"></span></span><span class="n">${n}</span></li>`).join("")}</ul></div>
<div><h3 class="sub-title">Latest movement</h3><ul class="movement">${latest.map(evLi).join("")}</ul></div>
<div><h3 class="sub-title">Dates ahead</h3><ul class="movement">${upcoming.map(evLi).join("")}</ul><p class="panel-foot">Full record in <a href="#sec-timeline">Changes over time</a>.</p></div>
</div></section>
${kmini ? `<section class="panel" aria-labelledby="cmp-h"><div class="panel-head"><h2 id="cmp-h" class="panel-title">How the leading instruments compare</h2><p class="panel-note">Selected rows from the tracker's own cross-cutting comparison. The full matrix, with every dimension and the assurance-layer table, is in <a href="${href("K")}">section K</a>.</p></div>${kmini}</section>` : ""}`;
