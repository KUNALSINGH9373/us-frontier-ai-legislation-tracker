...
(function(){
  ...
  function render(){
    ...
    Object.keys(L.cards).forEach(function(id){ var d = L.cards[id];
      ...
      d.addEventListener('click', function(ev){ ev.stopPropagation(); selected = (selected === id) ? null : id; pinned = null; hideTip(); render(); if (selected) setTimeout(function(){ panel.scrollIntoView({ block: 'start', behavior: 'smooth' }); }, 50); });
  ...
  function renderPanel(v){
    ...
    // The full tracker entry, verbatim, cloned from the tracker card on this page.
    if (n.kind !== 'actor'){ var card = document.getElementById(n.id); if (card){ var box = document.createElement('div'); box.className = 'mp-entry'; var bh = document.createElement('h4'); bh.textContent = 'From the tracker entry'; box.appendChild(bh);
        [].slice.call(card.querySelectorAll('dl.facts, dl.blocks, dd')).forEach(function(part){ if (part.tagName === 'DD' && part.closest('dl')) return; var cl = part.cloneNode(true); [].slice.call(cl.querySelectorAll('[id]')).forEach(function(x){ x.removeAttribute('id'); }); box.appendChild(cl); });
        if (card.classList.contains('pair')){ var dd = card.querySelector('dd'); if (dd){ var cl2 = dd.cloneNode(true); box.appendChild(cl2); } }
        panel.appendChild(box); } }
    else { var mentions = document.createElement('div'); mentions.className = 'mp-entry'; var mh = document.createElement('h4'); mh.textContent = 'Where the tracker names this actor'; mentions.appendChild(mh); var ml = document.createElement('ul'); ml.className = 'mp-mentions'; var seen = {}; M.edges.filter(function(e){ return e.from === id || e.to === id; }).forEach(function(e){ var other = e.from === id ? e.to : e.from; if (seen[other]) return; seen[other] = 1; var li = document.createElement('li'); var a = document.createElement('a'); a.href = byId[other].kind === 'actor' ? '#' : '#' + other; a.textContent = byId[other].short; li.appendChild(a); li.appendChild(document.createTextNode(' — ' + e.label)); var q = document.createElement('q'); q.textContent = e.quote; li.appendChild(q); ml.appendChild(li); }); mentions.appendChild(ml); panel.appendChild(mentions); }
    var count = document.createElement('p'); count.className = 'mp-count'; count.textContent = 'Relationships: the tracker states ' + edges.length + (edges.length === 1 ? ' link' : ' links') + ' for this node' + (edges.length !== M.edges.filter(function(e){ return e.from === id || e.to === id; }).length ? ' (some types are hidden by the current lens or legend)' : ''); panel.appendChild(count);
