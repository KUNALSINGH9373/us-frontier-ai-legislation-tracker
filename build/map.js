...
(function(){
  ...
  var focus = null, pinned = null, year = 'all', fit = true;
  ...
  $('#map-full').addEventListener('click', function(){ focus = null; pinned = null; render(); });
  ...
  var search = $('#map-q'); search.addEventListener('change', function(){ var v = search.value.trim().toLowerCase(); if (!v) return; var hit = M.nodes.filter(function(n){ return (n.short+' '+(n.title||'')).toLowerCase().indexOf(v) !== -1; })[0]; if (hit){ focus = hit.id; pinned = null; render(); } });
  ...
  function layout(v){
    ...
    layer.innerHTML = '';
    var cards = {};
    v.nodes.forEach(function(n){ var d = document.createElement('div'); d.className = 'map-node kind-' + n.kind + (yearOk(n) ? '' : ' offyear') + (n.id === focus ? ' focus' : ''); d.dataset.id = n.id; d.style.width = W + 'px';
  ...
  function render(){
    ...
    v.edges.forEach(function(e, i){ var a = L.xy[e.from], b = L.xy[e.to]; if (!a || !b) return; var ca = L.slot[byId[e.from].col], cb = L.slot[byId[e.to].col]; var x1, x2, y1 = a.y + a.h/2, y2 = b.y + b.h/2, d;
      ...
      hit.addEventListener('pointerleave', function(){ if (pinned === null) hideTip(); highlight(null, null); });
    ...
    Object.keys(L.cards).forEach(function(id){ var d = L.cards[id];
      ...
      d.addEventListener('pointerleave', function(){ highlight(null, null); });
      d.addEventListener('click', function(ev){ ev.stopPropagation(); focus = (focus === id && v.nodes.length > 1 && lens === 'all') ? null : id; pinned = null; hideTip(); render(); });
    ...
    $('#map-count').textContent = v.nodes.length + ' nodes · ' + v.edges.length + ' relationships' + (focus ? ' · focused on ' + byId[focus].short : '') + (year !== 'all' ? ' · instruments without a ' + (year==='2027+'?'2027 and later':year) + ' event are faded' : '');
    renderTable(v);
  }
  function renderPanel(v){ panel.innerHTML = ''; $('.map-layout').classList.toggle('has-panel', !!focus); if (!focus){ panel.hidden = true; return; } var n = byId[focus]; panel.hidden = false;
    var h = document.createElement('h3'); h.textContent = n.short; panel.appendChild(h);
    if (n.kind !== 'actor'){ var dl = document.createElement('dl'); [['Entry', n.title], ['Section', n.section ? 'Section ' + n.section + (n.sectionName ? ' · ' + n.sectionName : '') : ''], ['Jurisdiction', n.juris], ['Status class', n.status], ['Confidence', n.conf], ['Years with dated events', (n.years||[]).join(', ')]].forEach(function(kv){ if (!kv[1]) return; var dt = document.createElement('dt'); dt.textContent = kv[0]; var dd = document.createElement('dd'); dd.textContent = kv[1]; dl.appendChild(dt); dl.appendChild(dd); }); panel.appendChild(dl); var a = document.createElement('a'); a.href = '#' + n.id; a.className = 'viz-btn'; a.textContent = 'Open the tracker entry'; panel.appendChild(a); }
    else { var p = document.createElement('p'); p.className = 'map-actornote'; p.textContent = n.title || 'Actor named in the tracker.'; panel.appendChild(p); }
    var ul = document.createElement('ul'); ul.className = 'map-edges'; v.edges.forEach(function(e){ var li = document.createElement('li'); var i = document.createElement('i'); i.style.background = M.types[e.type].color; li.appendChild(i); var other = e.from === focus ? e.to : e.from; var dir = e.from === focus ? '→ ' : '← '; var b = document.createElement('b'); b.textContent = dir + byId[other].short; li.appendChild(b); var s = document.createElement('span'); s.textContent = e.label + (e.dashed ? ' (qualified)' : ''); li.appendChild(s); var q = document.createElement('q'); q.textContent = e.quote; li.appendChild(q); var src = document.createElement('a'); src.href = e.srcHref; src.textContent = 'source: ' + e.srcLabel + ' · ' + e.col; li.appendChild(src); ul.appendChild(li); }); panel.appendChild(ul);
    var back = document.createElement('button'); back.type = 'button'; back.className = 'viz-btn'; back.textContent = 'Show full map'; back.addEventListener('click', function(){ focus = null; pinned = null; render(); }); panel.appendChild(back);
  }
  ...
  var rendered = false;
  function maybe(){ if (root.hidden) return; if (!rendered){ var q = new URLSearchParams(location.search); year = q.get('year') || 'all'; var n = q.get('node'); if (n && byId[n]) focus = n; render(); rendered = true; } }
  var origRender = render; render = function(){ origRender(); var q = new URLSearchParams(location.search); if (focus) q.set('node', focus); else q.delete('node'); var qs = q.toString(); history.replaceState(null, '', location.pathname + (qs ? '?' + qs : '') + location.hash); };
